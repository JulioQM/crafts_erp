...
import craftserp.model.entities.VenFacturaDetalle;
...
//	public List<FacturaCab> findAllFacturaCab() {
	public List<VenFacturaCabecera> findAllFacturaCab() {
		List<VenFacturaCabecera> listado = managerDAO.findAll(VenFacturaCabecera.class, "numeroFactura", false);
		// recorremos las facturas cabecera para extraer los datos de los detalles:
		for (VenFacturaCabecera fc : listado) {
			for (VenFacturaDetalle fd : fc.getVenFacturaDetalles()) {
				fd.getCantidad();
			}
		}
		return listado;
	 ...
	 */
	public VenFacturaCabecera crearFacturaTmp() {
		Date date=new Date();
		VenFacturaCabecera facturaCabTmp = new VenFacturaCabecera();
		facturaCabTmp.setFechaEmision(new Timestamp(date.getTime()));
		facturaCabTmp.setVenFacturaDetalles(new ArrayList<VenFacturaDetalle>());
	 ...
	 * @throws Exception
	 */
	public void asignarClienteFacturaTmp(VenFacturaCabecera facturaCabTmp, String cedulaCliente) throws Exception {

		SegPersona cliente = null;
		if (cedulaCliente == null || cedulaCliente.length() == 0)
			throw new Exception("Error debe especificar la cedula del cliente.");
		try {
			cliente = managerClientes.findPersonaByCedula(cedulaCliente);
			if (cliente == null)
				throw new Exception("Error al asignar cliente.");
			facturaCabTmp.setSegPersona1(cliente);
			//facturaCabTmp.setSegPersona2(cliente);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @throws Exception
	 */
	private void calcularFacturaTmp(VenFacturaCabecera facturaCabTmp) throws Exception {
		double sumaSubtotales;
		double porcentajeIVA, valorIVA, totalFactura;
		// verificamos los campos calculados:
		sumaSubtotales = 0;
		for (VenFacturaDetalle det : facturaCabTmp.getVenFacturaDetalles()) {
			sumaSubtotales += det.getCantidad().intValue() * det.getPrecio().doubleValue();
		}
		...
		facturaCabTmp.setIva(new BigDecimal(valorIVA));
		facturaCabTmp.setTarifaCero(new BigDecimal(0));// no calculamos la base cero en este ejemplo.
	 ...
	 */
	public void agregarDetalleFacturaTmp(VenFacturaCabecera facturaCabTmp, Integer codigoProducto, Integer cantidad)
			throws Exception {
		ProProducto p;
		VenFacturaDetalle fd;
		...
		fd = new VenFacturaDetalle();
		fd.setDescripcion(p.getDescripcion());
		fd.setCantidad(cantidad);
		fd.setPrecio(p.getPrecioFinal());
		fd.setProProducto(p);
		facturaCabTmp.getVenFacturaDetalles().add(fd);
	 ...
	 */
	public void guardarFacturaTemporal(String codigoUsuario, VenFacturaCabecera facturaCabTmp) throws Exception {

		if (facturaCabTmp == null)
			throw new Exception("Debe crear una factura primero.");
		if (facturaCabTmp.getVenFacturaDetalles() == null || facturaCabTmp.getVenFacturaDetalles().size() == 0)
			throw new Exception("Debe ingresar los productos en la factura.");
		if (facturaCabTmp.getSegPersona1() == null)
			throw new Exception("Debe registrar el cliente.");

		// asignacion del usuario que crea la factura
		Date date=new Date();
		SegPersona usuario = managerSeguridad.findUsuarioById(codigoUsuario);
		facturaCabTmp.setSegPersona2(usuario);
		...
		for (VenFacturaDetalle det : facturaCabTmp.getVenFacturaDetalles()) {
			det.setVenFacturaCabecera(facturaCabTmp);
		}
	...
	/**
	 * Permite generar una factura a partir de un pedido de compra de un cliente.
	 * Este metodo reutiliza toda la logica de negocio que previamente fue
	 * implementada en ManagerFacturacion.
	 * 
...
//		FacturaCabecera facturaCabTmp = crearFacturaTmp();
